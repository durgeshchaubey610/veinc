 ...
 * Recent posts class.
 */
class Marjoram_Recent_Posts_Widget extends WP_Widget {
	/**
	...
	public function __construct() {
		$this->defaults = array(
			'title'     => esc_html__( 'Marjoram Recent Posts', 'marjoram' ),
			'number'    => 3,
			'show_date' => true,
		);
		parent::__construct(
			'marjoram-recent-posts',
			esc_html__( 'Marjoram: Recent Posts', 'marjoram' ),
			array(
				'description' => esc_html__( 'A widget that displays your recent posts from all categories or a category with thumbnails.', 'marjoram' ),
			)
		);
	}
					...
					<?php if ( has_post_thumbnail() ) : ?>
						<div class="recent-post_image">
							<a href="<?php esc_url(the_permalink()); ?>" rel="bookmark" title="<?php the_title_attribute(); ?>">
								<?php the_post_thumbnail( 'marjoram-recent' ); ?>
							</a>
					</div>
					<?php else: ?>
						<div class="recent-post_image">
							<a href="<?php esc_url(the_permalink()); ?>" rel="bookmark" title="<?php the_title_attribute(); ?>">
								<img src="<?php echo esc_url(get_template_directory_uri()); ?>/images/no-recent.png" alt="<?php esc_html_e( 'No Image', 'marjoram');?>"/>
							</a>
							</div>				
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e( 'Number of posts to show:', 'marjoram' ); ?></label>
