 ...
 * @subpackage Admin
 */
if ( ! class_exists( 'marjoram_info_page' ) ) {
	/**
	 * Singleton class used for generating the about page of the theme.
	 */
	class marjoram_info_page {
		/**
		 * Define the version of the class.
		 * @var string $version The marjoram_info_page class version.
		 */
		...
		/**
		 * The single instance of marjoram_info_page
		 * @var marjoram_info_page $instance The  marjoram_info_page instance.
		 */
		private static $instance;

		/**
		 * The Main marjoram_info_page instance.
		 * We make sure that only one instance of marjoram_info_page exists in the memory at one time.
		 * @param array $config The configuration array.
		 */
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof marjoram_info_page ) ) {
				self::$instance = new marjoram_info_page;
		...
		function register() {
			if ( ! empty( $this->menu_name ) && ! empty( $this->page_name ) ) {
				add_theme_page( $this->menu_name, $this->page_name, 'activate_plugins', 'marjoram-welcome', array(
					$this,
					'page_render',
				) );
			}
		...
		public function welcome_admin_notice() {
			...
			if ( ! empty( $this->notification ) ) {
				...
				if ( current_user_can( 'edit_theme_options' ) && !get_option( 'marjoram_notice_welcome' ) && 'themes.php' == $pagenow ) {

					echo '<div class="marjoram-info-about updated notice is-dismissible">';
					echo '<a class="notice-dismiss" href="' . esc_url(wp_nonce_url(remove_query_arg(array('activated'), add_query_arg('marjoram-hide-notice', 'welcome')), 'marjoram_hide_notices_nonce', '_marjoram_notice_nonce')) . '" style="z-index: 0;padding: 10px;text-decoration: none;" >';
					echo '<span class="screen-reader-text">' . esc_html__('Dismiss this notice.', 'marjoram'). '</span>';
					echo '</a>';
			...
			if ( ! empty( $welcome_title ) || ! empty( $welcome_content ) || ! empty( $this->tabs ) ) {
				...
				if ( ! empty( $this->tabs ) ) {
					...
					foreach ( $this->tabs as $tab_key => $tab_name ) {
						...
						echo '<a href="' . esc_url( admin_url( 'themes.php?page=marjoram-welcome' ) ) . '&tab=' . esc_attr( $tab_key ) . '" class="nav-tab ' . ( $active_tab == $tab_key ? 'nav-tab-active' : '' ) . '" role="tab" data-toggle="tab">';
		...
		*/
		public function theme_review() {
			echo '<div id="marjoram-support-content" class="feature-section two-col">';
		...
		public function getting_started() {
			...
			if ( ! empty( $this->config['getting_started'] ) ) {
				...
				if ( ! empty( $getting_started ) ) {
					...
					echo '<div id="marjoram-getting-started" class="feature-section three-col">';
		...
		public function free_pro() {
			...
			if ( ! empty( $free_pro ) ) {
				if ( ! empty( $free_pro['free_theme_name'] ) && ! empty( $free_pro['pro_theme_name'] ) && ! empty( $free_pro['features'] ) && is_array( $free_pro['features'] ) ) {
					echo '<div id="marjoram-free-pro" class="feature-section">';
					...
					if ( ! empty( $free_pro['pro_theme_link'] ) && ! empty( $free_pro['get_pro_theme_label'] ) ) {
						echo '<tr class="marjoram-free-pro-button">';
						echo '<td></td>';
		...
		public function style_and_scripts( $hook_suffix ) {
			...
			// enqueue css files
			if ( 'themes.php' === $pagenow || 'appearance_page_marjoram-welcome' == $hook_suffix ) {
				wp_enqueue_style( 'marjoram-about-page-css', get_template_directory_uri() . '/inc/theme-info/css/marjoram-info-backend.css' );
			}

			// enqueue js files
			if ( 'appearance_page_marjoram-welcome' == $hook_suffix ) {
				wp_enqueue_script( 'marjoram-about-page-js', ( get_template_directory_uri() . '/inc/theme-info/js/marjoram-info-backend.js' ), array( 'jquery' ), '', 'true' );
			}
		...
		public function hide_notice() {
			if (isset($_GET['marjoram-hide-notice']) && isset($_GET['_marjoram_notice_nonce'])) {
				if (!wp_verify_nonce($_GET['_marjoram_notice_nonce'], 'marjoram_hide_notices_nonce')) {
					wp_die(esc_html__('Action failed. Please refresh the page and retry.', 'marjoram'));
				}
				if (!current_user_can('edit_theme_options')) {
					wp_die(esc_html__('You do not have the necessary permission to perform this action.', 'marjoram'));
				}
				$hide_notice = sanitize_text_field( wp_unslash($_GET['marjoram-hide-notice']));
				update_option('marjoram_notice_' . $hide_notice, 1);
			}
		}
