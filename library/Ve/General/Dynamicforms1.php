...
foreach($array as $key){
	trim($key[5]);if($key[0]=='select')
			{
			$return .= '<select  alt="'.  $key[2] .'"  name="' . $key[1] . '" id="' .$idPrefix . $key[1] . '" class="' . $select . '">';											$i=0;foreach($key[3] as $opt=>$val){
			if(empty($key[5]) ){
				$return .= '<option value="' . (($i++==0) ? '' : $val) . '" label="' . $val . '">' . $val . '</option>';								
				}else{									$return .= '<option value="' . (($i++==0) ? '' : $val) . '" ';
...
$return="";	
foreach($array as $key){						
if($key[0]=='select'){				if(count($key[4])>0){		
			foreach($key[4] as $val){						if($val=="trim"){	
											$return .= 'if($(\'#' .$idPrefix . $key[1] . '\').val()=="" ){';										$return .= 'alert("Please select ' . $key[2] . ' ");$(\'#' .$idPrefix . $key[1] . '\').focus();return false;';									
											$return .= '}';						
}}}			
}else if($key[0]=='text'){
if(count($key[4])>0){
...
$return .= '}';						}else if($val=="numeric"){									 									
$return .= 'if(!CheckUnit($(\'#' .$idPrefix . $key[1] . '\').val())) {';										
$return .= 'alert("Please eneter ' . $key[2] . ' in numeric ! ");$(\'#' .$idPrefix . $key[1] . '\').focus();return false;';									$return .= '}';						}else if($val=="dob" || $val=="date" ){									 									
