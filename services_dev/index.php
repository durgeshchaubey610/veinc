...
if (isset($_POST['tag']) && $_POST['tag'] != "") {
    ...
    if ($tag == "login") {
        ...
        if ($user != FALSE) {
		 ...
		 $response = $db->getAccessMatrix($_POST['role'], $_POST['uid']);
		  echo json_encode($response);
        ...
        $work_order = $db->getWorkOrder($role_id, $uid, $cust_id, $filter, $woid_array,$workorderid, $wo_limit, $exsting_header, $max_wo_val,$wo_offset);
	
        if ($work_order != FALSE) {
            $response['error'] = FALSE;
            $response['wo_list'] = $work_order['work_rows'];
    ...
    } else if ($tag == "delete_attachment") {

		$wfId = $_POST['wfId'];
        $response = $db->deleteAttachment($wfId);
		echo $response;
		
    } else if ($tag == "upload_attachment") {

		$file_name = $_POST['file_name'];
		$woId = $_POST['woId'];
        $response = $db->uploadAttachment($file_name, $woId);
		echo $response;
        ...
        if ($buildings1 != FALSE) {
			...
			$response['auto_cnt'] = stripslashes($buildings[2]);
            echo json_encode($response);
        ...
        if ($company1 != FALSE) {
			...
			$response['auto_cnt'] = stripslashes($company[2]);
            echo json_encode($response);
        ...
        $company1= $db->getTenants($role_id, $uid, $cust_id, $tnadmin_id);
		$company = explode(":", $company1);

        if ($company != FALSE) {
            $response['error'] = FALSE;
            $response['tenantusers'] = stripslashes($company[0]);
			$response['auto_cnt'] = stripslashes($company[1]);
            echo json_encode($response);
        ...
        $category1 = $db->getCompanyCategory($role_id, $uid, $cust_id, $build_id);
		$category = explode(":", $category1);

        if ($category != FALSE) {
            $response['error'] = FALSE;
            $response['category'] = stripslashes($category[0]);
			$response['auto_cnt'] = stripslashes($category[1]);
            echo json_encode($response);
