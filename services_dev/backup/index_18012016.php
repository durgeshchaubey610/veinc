...
if (isset($_POST['tag']) && $_POST['tag'] != "") {
    ...
    if ($tag == "login") {
        ...
        $work_order = $db->getWorkOrder($role_id, $uid, $cust_id, $filter, $woid_array);
        ...
        if ($wo_labor != FALSE) {
            ...
            $response['totallabourcost'] = $wo_labor['totallabourcost'];
            echo json_encode($response);
        ...
        if ($wo_building_services != FALSE) {
            $response['error'] = FALSE;
            $response['wo_building_services'] = stripslashes($wo_building_services);
            echo json_encode($response);
        ...
        if ($wo_materials != FALSE) {
            $response['error'] = FALSE;
            $response['wo_materials'] = stripslashes($wo_materials);
            echo json_encode($response);
        ...
        if ($wo_outside_services != FALSE) {
            $response['error'] = FALSE;
            $response['wo_outside_services'] = stripslashes($wo_outside_services);
            echo json_encode($response);
