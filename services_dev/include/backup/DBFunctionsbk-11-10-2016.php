...
class DBFunctions {
	...
	function getAccessMatrix($role_id) {
			$result = array();
			$access_query = "SELECT * FROM location_access WHERE role= '" . $role_id. "' AND location_id =". 1; 
			$access_query = mysql_query($access_query);
			$access_result = mysql_fetch_array($access_query);
			$result['dline_access']['is_access'] =$access_result['is_access']; 
			$result['dline_access']['is_read'] =$access_result['is_read']; 
			$result['dline_access']['is_write'] =$access_result['is_write'];
			
			$access_query = "SELECT * FROM location_access WHERE role= '" . $role_id. "' AND location_id =". 3; 
			$access_query = mysql_query($access_query);
			$access_result = mysql_fetch_array($access_query);
			$result['create_access']['is_access'] =$access_result['is_access']; 
			$result['create_access']['is_read'] =$access_result['is_read']; 
			$result['create_access']['is_write'] =$access_result['is_write'];
			
			$access_query = "SELECT * FROM location_access WHERE role= '" . $role_id. "' AND location_id =". 4; 
			$access_query = mysql_query($access_query);
			$access_result = mysql_fetch_array($access_query);
			$result['closeWO_access']['is_access'] =$access_result['is_access']; 
			$result['closeWO_access']['is_read'] =$access_result['is_read']; 
			$result['closeWO_access']['is_write'] =$access_result['is_write'];
            return $result; 
	}
    ...
    public function getWorkOrder($role_id, $uid, $cust_id, $filter, $woid_array, $workorderid) {
		$ackbutton = '';
		$filter2 = '';
		$allstatus = array();
		$query = "SELECT * FROM schedule_status where status = '1'";
        ...
        if ($role_id == '7') { 
            ...
            while ($wo_row = mysql_fetch_array($wo_query)) {
                ...
                if ($wocnt % 2 == 0) {
					if($wo_status == 1) {
							...
							$statusResult = mysql_fetch_assoc($statusResult);
							//print_r($statusResult);
							if($statusResult['ckey'] !='') {
								$ackUrl = '"'.BASE_URL.'/appcomplete/index/woId/'.$statusResult['worder_id'].'/sId/'.$statusResult['schedule_id'].'/ckey/'.$statusResult['ckey'].'/statusChange/acknowledge/userId/'.$uid.'"'; 
								$ackbutton ="<br><button class='btn btn-danger btn-xs' onclick='acknowledgeWorkorder(event, ".$ackUrl.")' style='margin-top:8px; font-size:12px;' type='button'>Ack</button>";
							}
						} else {
						$ackbutton ='';
					}
                    $work_rows .= "<tr  class='vt_set_yellow'><td onclick='getDetail(" . $wo_id . ");'>" . $wocnt . "</td><td onclick='getDetail(" . $wo_id . ");'>" . $buildingName . "</td><td onclick='getDetail(" . $wo_id . ");'>" . $tenantName . "</td><td><span onclick='getDetail(" . $wo_id . ");' >" .$allstatus[$wo_status].'</span>'.$ackbutton. "</td><td onclick='getDetail(" . $wo_id . ");'>";
                    ...
                    $work_rows .= "</td></tr><tr class='vt_set_yellow' style='border-bottom: 1px solid #fff;'> <td colspan='1' onclick='getDetail(" . $wo_id . ");'>Description</td> <td colspan='4' onclick='getDetail(" . $wo_id . ");'>". $description ."</td></tr>";
                } else {
                    ...
                    $work_rows .= "<tr  class='vt_set_purple'><td onclick='getDetail(" . $wo_id . ");'>" . $wo_number . "</td><td onclick='getDetail(" . $wo_id . ");'>" . $buildingName . "</td><td onclick='getDetail(" . $wo_id . ");'>" . $tenantName . "</td><td><span onclick='getDetail(" . $wo_id . ");'>" .$allstatus[$wo_status].'</span>'.$ackbutton. "</td><td onclick='getDetail(" . $wo_id . ");'>";
                    ...
                    $work_rows .= "</td></tr><tr class='vt_set_purple' style='border-bottom: 1px solid #fff;'> <td colspan='1' onclick='getDetail(" . $wo_id . ");'>Description</td> <td colspan='4' onclick='getDetail(" . $wo_id . ");'>". $description ."</td></tr>";
                }
            }
        } 
		else if ($role_id == '5') {
            ...
            while ($wo_row = mysql_fetch_array($wo_query)) {
                ...
                if ($wocnt % 2 == 0) {
                    ...
                    $work_rows .= "<tr  class='vt_set_yellow'><td onclick='getDetail(" . $wo_id . ");'>" . $wo_number . "</td><td onclick='getDetail(" . $wo_id . ");'>" . $buildingName . "</td><td onclick='getDetail(" . $wo_id . ");'>" . $tenantName . "</td><td><span onclick='getDetail(" . $wo_id . ");'>" .$allstatus[$wo_status].'</span>'.$ackbutton. "</td><td onclick='getDetail(" . $wo_id . ");'>";
                    $schedule_query = "SELECT `priority_schedule`.* FROM `priority_schedule` WHERE (priority_id = '" . $wo_row['prioritySchedule'] . "') AND (start_status = '" . $wo_status . "')";
                    $schedule_sql = mysql_query($schedule_query);
                    $schedule = mysql_fetch_array($schedule_sql);
            ...
            if ($build_ID == '') {
                ...
                while ($wo_row = mysql_fetch_array($wo_query)) {
                    ...
                    if ($wocnt % 2 == 0) {
                        ...
                        $work_rows .= "<tr  class='vt_set_purple' style='border-bottom: 1px solid #fff;'><td onclick='getDetail(" . $wo_id . ");'>" . $wo_number . "</td><td onclick='getDetail(" . $wo_id . ");'>" . $buildingName . "</td><td onclick='getDetail(" . $wo_id . ");'>" . $tenantName . "</td><td><span onclick='getDetail(" . $wo_id . ");'>" .$allstatus[$wo_status].'</span>'.$ackbutton. "</td><td onclick='getDetail(" . $wo_id . ");'>";
                        $schedule_query = "SELECT `priority_schedule`.* FROM `priority_schedule` WHERE (priority_id = '" . $wo_row['prioritySchedule'] . "') AND (start_status = '" . $wo_status . "')";
                        $schedule_sql = mysql_query($schedule_query);
                        $schedule = mysql_fetch_array($schedule_sql);
    ...
    function getWorkOrderDetail($role_id, $uid, $cust_id, $woid) {
        $query = "SELECT `wo`.*, `wop`.*, `t`.`tenantName`, `t`.`tenantContact`, `bu`.`build_id`, `bu`.`buildingName`, `cat`.`categoryName`, `cat`.`send_email`, `pt`.`priorityName`, `pt`.`pid`, `u`.`firstName`, `u`.`lastName`, `u`.`email`, `u`.`phoneNumber`, `tu`.`suite_location` AS `tenant_suite`,`wf`.`file_name` as attachment
			FROM `work_order` AS `wo`
			INNER JOIN `tenant` AS `t` ON t.id = wo.tenant
			LEFT JOIN `buildings` AS `bu` ON bu.build_id = wo.building
			LEFT JOIN `category` AS `cat` ON cat.cat_id = wo.category
			...
			WHERE (wo.woId='" . $woid . "') order by wf.wfId desc";
        $sql = mysql_query($query);

        $data = mysql_fetch_array($sql);
		
		$query_closed = "SELECT status_closed from wo_parameter where building=".$data['build_id'];
		$sql_closed = mysql_query($query_closed);
		$status_closed = mysql_fetch_assoc($sql_closed);
    ...
    function getLabor($woid) {
        ...
        if ($cnt > 0) {
            while ($row = mysql_fetch_array($sql)) {
                ...
                if (($evenodd % 2)) {
                    $evenodd++;
                    $html .= "<tr style=\"background:#f9f9f9;\" onclick=\"trEventClick()\"><td>" . $evenodd . "</td><td>" . $row['firstName'] . " " . $row['lastName'] . "</td><td>$" . $row['charge_hour'] . "</td><td>" . $row['description'] . "</td><td>" . $row['job_time'] . "<div class='select_action'>
								<ul>
								   <li><a onclick=\"updateLabourDataTemp('" . $row['emp_id'] . "', '" . $row['charge_hour'] . "', '" . $rate_charge . "', '" . $row['brid'] . "','" . $row['job_time'] . "', '" . $row['woId'] . "', '" . $row['lid'] . "')\">Edit</a></li>
    ...
    function getBuildingServices($woid) {
        ...
        if ($cnt > 0) {
            while ($row = mysql_fetch_array($sql)) {
                ...
                if (($evenodd % 2)) {
                    $evenodd++;
                    $html .= "<tr style=\"background:#f9f9f9;\" onclick=\"trEventClick()\"><td>" . $evenodd . "</td><td>" . $row['service_name'] . "</td><td>$" . $row['charge'] . "</td><td>" . $row['unit_measure'] . "</td><td>" . $row['amount_requested'] . "</td><td>" . $row['comment'] . "	<div class='select_action'>
								<ul>
    ...
    function getMaterials($woid) {
        ...
        if ($cnt > 0) {
            while ($row = mysql_fetch_array($sql)) {
                ...
                if (($evenodd % 2)) {
                    $evenodd++;
                    $html .= "<tr style=\"background:#f9f9f9;\" onclick=\"trEventClick()\"><td>" . $evenodd . "</td><td>" . $row['description'] . "</td><td>$" . $row['cost'] . "</td><td>" . $row['quantity'] . "</td><td>" . $row['markup'] . "</td><td>" . $tax . "<div class='select_action'>
							<ul>
    ...
    function getOutsideServices($woid) {
        ...
        if ($cnt > 0) {
            while ($row = mysql_fetch_array($sql)) {
                ...
                if (($evenodd % 2)) {
                    $evenodd++;
                    $html .= "<tr style=\"background:#f9f9f9;\" onclick=\"trEventClick()\"><td>" . $evenodd . "</td><td>" . $row['company_name'] . "</td><td>" . $row['job_description'] . "</td><td>$" . $row['job_cost'] . "</td><td>" . $row['markup'] . "</td><td>" . $tax . "<div class='select_action'>
							<ul>
    ...
    function getNotes($woid) {
        ...
        if ($cnt > 0) {
            while ($row = mysql_fetch_array($sql)) {
                ...
                if (($evenodd % 2)) {
                    $evenodd++;
                    $html .= "<tr style=\"background:#f9f9f9;\" onclick=\"trEventClick()\"><td>" . $evenodd . "</td><td>" . $date . "</td><td>" . $row['note'] . "</td><td>" . $internal . "<div class='select_action'>
												<ul>
    ...
    function updateWorkOrderStatus($woid, $role_id, $uid, $cust_id, $wo_status, $new_wo_status, $building_id) {
		...
		$sql_billable = mysql_query($query_billable);
		$status_billable = mysql_fetch_assoc($sql_billable);
		/******************* set timezone through building id ****************************/
        $ob = new Helper();
    ...
    function createSaveWorkOrder($role_id, $uid, $cust_id, $buildings_list, $company_list, $tenantusers_list, $category_list, $current_date_field, $current_time_field, $internal_wo, $wo_request, $wo_notes, $file_name='') {
		...
		if($wo_notes!='') {
		    ...
		    $res_notes = mysql_query($insert_notes);
		}
