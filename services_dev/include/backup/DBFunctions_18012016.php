...
class DBFunctions {
    ...
    function getWorkOrderDetail($role_id, $uid, $cust_id, $woid) {
        $query = "SELECT `wo`.*, `wop`.*, `t`.`tenantName`, `t`.`tenantContact`, `bu`.`buildingName`, `cat`.`categoryName`, `cat`.`send_email`, `pt`.`priorityName`, `pt`.`pid`, `u`.`firstName`, `u`.`lastName`, `u`.`email`, `u`.`phoneNumber`, `tu`.`suite_location` AS `tenant_suite`
			FROM `work_order` AS `wo`
			INNER JOIN `tenant` AS `t` ON t.id = wo.tenant
			LEFT JOIN `buildings` AS `bu` ON bu.build_id = wo.building
			LEFT JOIN `category` AS `cat` ON cat.cat_id = wo.category
    ...
    function getLabor($woid) {
        ...
        return compact("html","totallabourcost");
