...
class DBFunctions {
	...
	function getAccessMatrix($role_id, $uid='') {
			...
			if($uid != '' && $role_id != '') {
				...
				$access_query_loc1 = mysql_query($access_query_loc1);
				$access_result_loc1 = mysql_fetch_array($access_query_loc1);
				...
				$access_query_loc3 = mysql_query($access_query_loc3);
				$access_result_loc3 = mysql_fetch_array($access_query_loc3);
				...
				$access_query_loc4 = mysql_query($access_query_loc4);
				$access_result_loc4 = mysql_fetch_array($access_query_loc4);
    ...
    public function getWorkOrder($role_id, $uid, $cust_id, $filter, $woid_array, $workorderid, $woLimit='', $exsting_header='', $return_max_wo_val='',$wo_offset='') {
		...
		$color_code = array('00FFFF','FF0000','FF00FF','800000','008000','800080','808000','0000FF','00FF00','000080','FFFF00','808000','008080','FFA500');$building_color = array();
        ...
        $work_rows = "";
            ...
            if ($build_ID == '') {
				...
				$max_wo_query = mysql_query($max_wo);
				$max_wo_val = mysql_fetch_assoc($max_wo_query);
                ...
                while ($wo_row = mysql_fetch_array($wo_query)) {
					...
					if($wo_row['days_old'] == 0 ) {  
						...
						$checkfield = 1;
						$oldName = 'Today';  
						$newwoqueryex = mysql_query($newwoquery);
						$newwoqueryresult = mysql_fetch_array($newwoqueryex);
	...
	{	if($wfId != '') {
			try {
				...
				WHERE (wfId='" . $wfId . "')";
				$filesql = mysql_query($filequery);
				$filedata = mysql_fetch_assoc($filesql);
    ...
    function getWorkOrderDetail($role_id, $uid, $cust_id, $woid) {
        ...
        $filesql = mysql_query($filequery);
		$filename=array();
        while($filedata = mysql_fetch_assoc($filesql)) {
    ...
    function updateWorkOrderStatus($woid, $role_id, $uid, $cust_id, $wo_status, $new_wo_status, $building_id) {
		...
		$curstatusrow = mysql_query($curstatusquery);
		$curstatusval = mysql_fetch_assoc($curstatusrow);
	...
	function checkPushId($uid, $pushId, $device) {
		...
		$data = mysql_fetch_assoc($query_sql); 
		if(!$data) {
	...
	function removepushId($uid, $pushId) {
		...
		$device_token = mysql_fetch_assoc($row);
	...
	function sendBadgeAfterLogin($uid,$pushid,$device) {
			...
			if($device == 1) {
				...
				$tPort = 2195;
				//$tCert = '/home3/voctech/public_html/visionworkorders.com/services_dev/include/ck2.pem';
				$tCert = 'ck2.pem';
...
// Close the Connection to the Server.
	...
	function sendOnlyBadge($buildings_list,$status)
	{
		 ...
		 $iosPushIdList = array();
		 while($data = mysql_fetch_assoc($query_ins)) {
		 ...
		 if (!empty($userList)) {
			 ...
			 $queryPushSql =  mysql_query($queryPush);
			 
			 while($queryPushData = mysql_fetch_assoc($queryPushSql)) {
			...
			$queryIosPushSql =  mysql_query($queryPush);
			while($queryIosPushData = mysql_fetch_assoc($queryIosPushSql)) { 
...
// Close the Connection to the Server.
	...
	public function getWorkorderbystatus($user_id,$status) { 
		if($user_id != '') {
		    ...
		    $buildingrow = mysql_query($select);
			
			while($building = mysql_fetch_assoc($buildingrow)) {  
			...
			$workcountow = mysql_query($select);
			$countworkorder = mysql_fetch_assoc($workcountow);
	...
	public function sendPushNotification($pushInfo) {
		 ...
		 $sqlBuilding = mysql_query($query);
		 $buildingName = mysql_fetch_assoc($sqlBuilding);
		 $buildingName = $buildingName['buildingName'];
		 
		 $query = "SELECT categoryName FROM category WHERE cat_id = '" . $category_list . "'";
		 $selCategory = mysql_query($query);
		 $categoryName = mysql_fetch_assoc($selCategory);
