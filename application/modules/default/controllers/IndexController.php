...
class IndexController extends Ve_Controller_Base {
    ...
    public function sendResetInstruction($userDetail) {
        try {
            ...
            $companyDetail = $accountModel->getcompany($userDetail[0]['cust_id']);
            $companyLogo = $companyDetail[0]['company_logo'];
            // assign valeues
            ...
            $emial_template->assign('userName', $userDetail[0]['userName']);
            $emial_template->assign('companyLogo', $companyLogo);
    ...
    public function resetpasswordAction() {
        ...
        if (!empty($passkey)) {
            ...
            if ($userDetails != false) {
                ...
                $companyDetail = $accountModel->getcompany($userData['cust_id']);
                $companyLogo = $companyDetail[0]['company_logo']; 
        ...
        if ($this->getRequest()->getMethod() == "POST") {
            try {
                $data = $this->getRequest()->getPost();
                $userModel->updateresetpassword(md5($data['repwd']), $passkey);
                //$userDetails = $userModel->passkey($passkey);
                $message = "Your password have reset successfully.";
                $show_form_status = false;
                $show_login_url = true;
                $this->sendUpdateResetInstruction($userDetails, $data['repwd']);
            } catch (Exception $e) {
        ...
        $this->view->companyLogo = $companyLogo;
    }
