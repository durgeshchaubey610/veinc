...
class DashboardController extends Ve_Controller_Base {
    ...
    public function workorderAction() {
        ...
        if($show != ""){
            setcookie('show_limit_list', $show, 2147483647, '/');
        }else{
            $show =  $_COOKIE['show_limit_list'];
        }
        ...
        if ($companyListing != '') {
            ...
            if ($build_ID == '') {
                ...
                $wolist = $woModel->getWorkOrderByBuilIds($buildIds, $order, $dir, $search_array,$show);
            } else {

                $wolist = $woModel->getBuildingWorkOrder($build_ID, $order, $dir, $search_array,$show);
            }
            /*
            }else{
            //echo print_r($wolist);
        ...
        $paginator = $pageObj->fetchPageDataRaw($wolist, $page, $show);
