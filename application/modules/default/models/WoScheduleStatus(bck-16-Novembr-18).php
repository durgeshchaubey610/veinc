...
class Model_WoScheduleStatus extends Zend_Db_Table_Abstract {
    ...
    public function getCurrentWoSchedule($buildingId) {
        ...
        $select = $db->select()
                ...
                ->joinLeft(array('pp' => 'priority'), 'ws.priority_id = pp.pid', array('parent_priority_status' => 'pp.status', 'parent_email_status' => 'pp.email_status', 'building_id' => 'pp.building_id'))
                ->where('ps.status = ?', '1')
                ->where('ws.current_status = ?', '1')
                ->where('pp.building_id = ?', $buildingId);
    ...
    public function getWorkOrderReminder() {
        ...
        $build_id = $build->getBuildingIdOnly();

        foreach ($build_id as $buildingid) {

            $currentSchedule = $this->getCurrentWoSchedule($buildingid->build_id); //echo '<pre>';	 print_r($currentSchedule);
            if ($currentSchedule) {
                ...
                $uploaddir = IMAGE_UPLOAD_DIR . '\cron';
