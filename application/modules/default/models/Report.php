...
class Model_Report extends Zend_Db_Table_Abstract {
    ...
    public function getReport($accounts = null, $dashboard_menu = null, $parent_id=null) {
        $db = Zend_Db_Table::getDefaultAdapter();
        try {
            $select = $db->select()
            ...
            if ($parent_id != null) {
                $select->where('dash.parent_id=?', $parent_id);
            }
    ...
    public function getReportDedail($dashboard_menu) {
        $db = Zend_Db_Table::getDefaultAdapter();
        $select = $db->select()
                ->from(array('t1' => 'report'))
                ->where('dashboard_menu=?', $dashboard_menu)
                ->where('status=?', '1');        
