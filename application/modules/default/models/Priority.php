...
class Model_Priority extends Zend_Db_Table_Abstract  {
	...
	/*
        ...
        */
		...
		public function getPrioritByNameDuplicate($priorityName,$buildId, $orderby){
			if($priorityName!='' && !empty($priorityName)){
				$select=$this->select()->where("priorityName LIKE '%".$priorityName."%'") ;
				if($buildId!=''){$select=$select->where('building_id=?',$buildId);}
		...
		public function deletePreorityByBId($bId){
			if(!empty($bId)){
				try{
					$bpreorityList = $this->getBuildingPriorityList($bId);
					if($bpreorityList){
						foreach($bpreorityList as $bpl){
							$this->deletePreority($bpl['pid']);
						}
					}
				}catch(Exception $e){
				}
		...
		public function getPriorityByCompany($cust_id,$bid) {
				$db = Zend_Db_Table::getDefaultAdapter(); 
				$select = $db->select()
						 ->from(array('pr' => 'priority'))
						 ->joinInner(array('b' => 'buildings'), 'pr.building_id = b.build_id', array('build_id'))
						 ->where('b.cust_id = ?', $cust_id)
						 ->where('pr.global_template = ?', '1')
						 ->where('pr.building_id != ?', $bid)
						 ->where('pr.status = ?', '1');	  				 
					$res = 	 $db->fetchAll($select); 
				return ($res && sizeof($res)>0)? $res : false ;
