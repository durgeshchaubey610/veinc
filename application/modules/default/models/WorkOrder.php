   ...
   /* Get all work order list */
	...
	public function getWorkOrderByBuilIds($buildingIds,$order,$dir,$search_array=array(),$page, $show){
		...
		if(!empty($buildingIds)){
			...
			$db = Zend_Db_Table::getDefaultAdapter();
			$select = $db->select()
                      ->from(array('wo' => 'work_order'), array('count(woId) as c'))
                      ->joinInner(array('t' => 'tenant'),'t.id = wo.tenant',array())
                      ->joinLeft(array('bu'=>'buildings'),'bu.build_id = wo.building',array())
                      ->joinLeft(array('cat'=>'category'),'cat.cat_id = wo.category',array())                      
                      ->joinLeft(array('wop'=>'work_order_update'),'wop.wo_id = wo.woId AND wop.current_update = 1',array())
                      ->joinLeft(array('u'=>'users'),'wo.create_user = u.uid',array())                      
                      ->where('wo.building in ('.implode(",", $buildingIds).')');
...
///////////////////////////// html type 6
    ...
    public function getActiveWorkOrderByBId($bid) {
		...
		public function getWorkOrderByBuilIdsFilter($buildingIds,$tid1,$order,$dir,$search_array=array(),$tid,$showbatch,$lastbatch,$searchtodate,$searchfromdate){
		if(!empty($buildingIds)){
			//$select = $this->select()->where('status=?','1') ;
			//print_r($search_array);
			//exit;
	...
	public function getBuildingWorkOrderFilter($buildID, $tid1,$order,$dir,$search_array=array(),$tid,$showbatch,$lastbatch,$searchtodate,$searchfromdate){
		if($buildID){
			//$select = $this->select()->where('status=?','1') ;
			//$orderBy = $order.''.$dir;
			$db = Zend_Db_Table::getDefaultAdapter();
