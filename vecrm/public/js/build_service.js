...
function deleteService(bsid){
	...
	var return_value = prompt("For Deleting Building Service, Enter Yes in Capital letter.");
	    if(return_value!=null){	
				...
				$('#service_error').html('You have entered wrong word.');
			}
		}
}
         ...
         if(submit_flag==false){			 
			...
			return false; 
				...
				$.ajax({
						...
						data        : {
							            service_name:service_name,unit_measure:unit_measure,status:status,
							            cost:cost,minimum:minimum,bsid:bsid,building:bId,
							          },
						success     : function( data ) {
							$('.loader').hide(); 
							var content = $.parseJSON(data);
