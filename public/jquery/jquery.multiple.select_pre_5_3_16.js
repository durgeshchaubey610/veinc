...
(function ($) {
    ...
    function MultipleSelect($el, options) {
        ...
        this.$choice = $('<button type="button" class="ms-choice">Select<span class="placeholder">'+
        ...
        this.selectItemName = 'name="search_chkboxstatus[]' + name + '" class="multicheckdd" ';
    }
    ...
    MultipleSelect.prototype = {
        ...
        optionToHtml: function (i, elm, group, groupDisabled) {
            ...
            if ($elm.is('option')) {
                ...
                if ((this.options.blockSeparator > "") && (this.options.blockSeparator == $elm.val())) {
                    var li = $(
                        '<li' + clss + style + '>',
                        '<span class="' + this.options.blockSeparator + (disabled ? 'disabled' : '') + '">',
                        '</span>',
                        '</li>'
                    );
                    li.find('span').append(document.createTextNode(text));
                    return li;
                } else {
                    var li = $(
                        '<li' + clss + style + '>' +
                ...
                group.append(
                    '<li class="group">' +
                    '<span class="optgroup' + (disabled ? ' disabled' : '') + '" data-group="' + _group + '">' +
        ...
        events: function () {
            ...
            var span = this.$el.parent().closest('span')[0] || $('span[for=' + this.$el.attr('id').split(':').join('\\:') + ']')[0];

            if (span) {
                $(span).off('click').on('click', function (e) {
                    if (e.target.nodeName.toLowerCase() !== 'span' || e.target !== this) {
                        return;
                    }
                    toggleOpen(e);
