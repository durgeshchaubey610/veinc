...
function showAddCoiVendor(vId){
	if(vId!=''){
		$('.loader').show();
		$.ajax({
				url         : baseUrl+"coilist/addvendor",
				type        : "post",
				datatype    : 'json',
				data        : {vId:vId},
				success     : function( data ) {
         ...
         if(submit_flag==false){			 
			...
			return false; 
			  ...
			  if (typeof manual !== 'undefined') {
                ...
                userData.append('coi_au_date_to', expiration_date);
				userData.append('tenant_Id', vendor_id);
				userData.append('tenant_number', account_number);
				userData.append('building_id', buliding_id);

			    $('.loader').show();
				$.ajax({
						url         : baseUrl+"coilist/savevendor",
						type        : "post",
						datatype    : 'json',
						cache: false,
                        contentType: false,
...
function showEditCoiVendor(vid){
	if(vid!=''){
		$('.loader').show();
		$.ajax({
				url         : baseUrl+"coilist/editvendor",
				type        : "post",
				datatype    : 'json',
				data        : {vid:vid},
				success     : function( data ) {
...
function updateCoiVendorList(cid){
	parent.CheckForSessionpop(baseUrl);
	if(cid!=''){
		//alert('hello');
		var submit_flag=true;
         ...
         if(submit_flag==false){			 
			...
			return false; 
			   ...
			   if (typeof manual !== 'undefined') {
			    ...
			    $('.loader').show();
				$.ajax({
						url         : baseUrl+"coilist/updatevendor",
						type        : "post",
						datatype    : 'json',
						cache       : false,
                        contentType : false,
...
function deleteCoiVendorList(vid){
	var check_delete='YES';
	//var con = confirm("Do you really want to delete category?");
	jPrompt('For Deleting Coi List, Enter Yes in Capital letters.', '', 'Alert', function(return_value) {
	    if(return_value!=null){	
				...
				$.ajax({        
					type: "POST",
					url: baseUrl + "coilist/deletecoivendor",
					dataType:'json',
					data: {
						vid: vid		
					},					
...
function sendcoiemail(tid, cid, tname) {
    ...
    $.ajax({
        type: "POST",
        url: baseUrl + 'coilist/sendcoiemail',
        ...
        success: function (data) {
            ...
            if (data == true) {
                ...
                //alert('There was an error');			
                $('.error-txt').html('Error occured during sending COI letters.');
            }
...
function sendcoivendoremail(tid, cid, tname) {	
    ...
    $.ajax({
        type: "POST",
        url: baseUrl + 'coilist/sendcoivendoremail',
        data: {tid: tid, cid: cid},
        success: function (data) {
            $('.loader').hide();
            if (data == true) {
