...
else if($.trim(postalCode).length < 5 ) {$('.postal-err').html("5 Digits Postal Code Required"); $('#postalCode').addClass('inputErr');$('#postalCode').focus();return false;  }
...
else if($.trim(phoneNumber).length < 12) { $('.phone-err').html("10 Digits Phone Number Required"); $('#phoneNumber').addClass('inputErr'); $('#phoneNumber').focus();return false; }
else{$('.phone-err').html(''); $('#phoneNumber').removeClass('inputErr');}
if($.trim(faxNumber).length < 12 && $.trim(faxNumber).length >= 1){ $('.fax-err').html("10 Digits Fax Number Required"); $('#faxNumber').addClass('inputErr');$('#faxNumber').focus();return false;}
else{$('.fax-err').html(''); $('#faxNumber').removeClass('inputErr');}
if($.trim(timezone) == ''){$('.tzone-err').html("Time Zone Required"); $('#timezone').addClass('inputErr');$('#timezone').focus();return false;}
...
else if($.trim(billpostalCode).length < 5 ) {$('.billpostalCode-Err').html("5 Digits Billing Postal Code Required"); $('#billpostalCode').addClass('inputErr');$('#billpostalCode').focus();return false;  }
...
else if($.trim(billphoneNumber).length < 12) { $('.billphoneNumber-Err').html("10 Digits Billing Phone Number Required"); $('#billphoneNumber').addClass('inputErr'); $('#billphoneNumber').focus();return false; }
else{ $('.billphoneNumber-Err').html(''); $('#billphoneNumber').removeClass('inputErr');}

if($.trim(billfaxNumber).length < 12 && $.trim(billfaxNumber).length >= 1){$('.billfaxNumber-Err').html("Billing Fax Number Required"); $('#billfaxNumber').addClass('inputErr');$('#billfaxNumber').focus();return false;}
...
function finalSubmit()
{
	...
	if($.trim(fname) == ''){
		...
		if(!chkfocus) $('#fname').focus(); chkfocus=1;
		submit_flag = 1;
		}
	else{
		$('.fnameErr').html('');
	...
	if($.trim(lname) == ''){
		...
		if(!chkfocus) $('#lname').focus(); chkfocus=1;
		submit_flag = 1;
		}
	else{
		$('.lnameErr').html(''); 
	...
	if($.trim(uPass) == '' && check_auto == false){
		...
		if(!chkfocus) $('#uPass').focus(); chkfocus=1;
		submit_flag = 1;
		}
	else{
		$('.uPassErr').html('');
	...
	if($.trim(uConfPass) == '' && check_auto == false){
		...
		if(!chkfocus) $('#uConfPass').focus(); chkfocus=1;
		submit_flag = 1;
	}
	else{
		$('.uConfErr').html('');
	...
	if($.trim(phone) == ''){
		...
		if(!chkfocus) $('#phone').focus(); chkfocus=1;
		submit_flag = 1;
		} 
		else if($.trim(phone).length < 12) {
		$('.phoneErr').html("10 Digits Phone Number Required");
		$('#phone').addClass('inputErr');
		if(!chkfocus) $('#phone').focus(); chkfocus=1;
		submit_flag = 1;
		}
 ...
 function isNumberKey(evt)
{
 ...
 $(function() {
			$("input#postalCode").mask("?99999",{autoclear: false});
